...
router.post('/add/:project_id', function(req, res, next) {
    let tasks = new taskModel ({
     ...
     created_by:req.body.created_by
      ...
      tasks.save(function(err, tasks){
        if (err)
        res.send({error:'task not added!!'})  
        else{
        res.send({data:{title:tasks.title,type:tasks.type,due_date:tasks.due_date, color:tasks.color,members:tasks.members,created_by:tasks.created_by,}});
  ...
  router.delete('/deleteTask', function(req, res, next) {
    ...
    taskModel.findByIdAndDelete({_id:req.body.id}, function(err, response){
      ...
      res.send({resultsFound:response.length,task_id: response});
